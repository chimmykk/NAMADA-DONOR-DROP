...
export async function markBlocksAsScraped(blocks) {
  ...
  for (let i = 0; i < blocks.length; i += BATCH_SIZE) {
    ...
    const values = batch.map((block) => [
      block.blockNumber,
      block.transactionsFound
    ]).flat();

    const placeholders = batch.map((_, j) => {
      const offset = j * 2; // 2 parameters per row
      return `($${offset + 1}, $${offset + 2}, CURRENT_TIMESTAMP)`;
    }).join(',');
